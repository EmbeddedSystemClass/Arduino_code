...
const double bat_discharged = 6.300; //absolute cutoff to avoid damage to battery, load disconnected at this point
const double bat_warning = 6.500; //optional implementation to warn connected controllers of low battery voltage

const double bat_stage1 = 6.600; //bulk charge cuts off after stage 1 limit
double pwm_stage1 = 191.250; //75%

const double bat_stage2 = 6.700; //PWM charge rate decreases to trickle for stage 2 until full charge achieved
double pwm_stage2 = 107.500;  //50%

double pwm_final = 25.5;  //10%
const double bat_full = 6.75; //absolute maximum cutoff to avoid damage to battery, solar panels disconnected
...
void loop()
{
  ...
  String inc = "";
  char c;
  Serial.println("Enter new PWM value? (enter as %)");
